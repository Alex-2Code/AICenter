<template>
	<view>
		...
		<view style="width: 520rpx;height: 60rpx; padding-top: 32rpx; text-align: center; margin: 0 auto;">
			<u-button type="primary" shape="circle" @click="chooseImg">上传火车票</u-button>
		</view>
		<view v-if="show">
			...
			<view style="padding-left: 20rpx;padding-right: 30rpx;">
				<u-form :label-style="form">
					<u-form-item :rightIconStyle="{ color: '#888', fontSize: '32rpx' }" :border-bottom="true" label="车 次">
						<u-input :disabled="true" :border="false" placeholder="" v-model="form.num"></u-input>
					</u-form-item>
					<u-form-item :rightIconStyle="{ color: '#888', fontSize: '32rpx' }" :border-bottom="true" label="始 发">
						<u-input :disabled="true" :border="false" placeholder="" v-model="form.start"></u-input>
					</u-form-item>
					<u-form-item :rightIconStyle="{ color: '#888', fontSize: '32rpx' }" :border-bottom="true" label="到 达">
						<u-input :disabled="true" :border="false" placeholder="" v-model="form.end"></u-input>
					</u-form-item>
					<u-form-item :rightIconStyle="{ color: '#888', fontSize: '32rpx' }" :border-bottom="true" label="售 站">
						<u-input :disabled="true" :border="false" placeholder="" v-model="form.sales"></u-input>
					</u-form-item>
					...
					<u-form-item :rightIconStyle="{ color: '#888', fontSize: '32rpx' }" :border-bottom="true" label="票 价">
						<u-input :disabled="true" :border="false" placeholder="" v-model="form.rates"></u-input>
					</u-form-item>
					<u-form-item :rightIconStyle="{ color: '#888', fontSize: '32rpx' }" :border-bottom="true" label="座 次">
						<u-input :disabled="true" :border="false" placeholder="" v-model="form.seat"></u-input>
					</u-form-item>
					<u-form-item :rightIconStyle="{ color: '#888', fontSize: '32rpx' }" :border-bottom="true" label="座 位">
						<u-input :disabled="true" :border="false" placeholder="" v-model="form.seat_num"></u-input>
					</u-form-item>
					...
					<u-form-item :rightIconStyle="{ color: '#888', fontSize: '32rpx' }" :border-bottom="true" label="ID号">
						<u-input :disabled="true" :border="false" placeholder="" v-model="form.id_num"></u-input>
					</u-form-item>
					<u-form-item :rightIconStyle="{ color: '#888', fontSize: '32rpx' }" :border-bottom="true" label="车 票">
						<u-input :disabled="true" :border="false" placeholder="" v-model="form.ticket_num"></u-input>
					</u-form-item>
				</u-form>
			</view>
			<view style="padding-top: 30rpx;background-color: #f2f2f2;">
				<u-cell-group style="width: 100%;"><u-cell-item title="应用场景" title-width="60rpx" value="财务报销、账单记录" :arrow="false"></u-cell-item></u-cell-group>
			</view>
		</view>
	</view>
</template>
...
export default {
	...
	methods: {
		...
		getInfo: function(tk) {
			// 发起请求获取信息
			uni.request({
				url: 'https://aip.baidubce.com/rest/2.0/ocr/v1/train_ticket?access_token=' + tk,
				// 请求方式
				method: 'POST',
				// 请求头
				header: {
		...
		getJudge: function(num,data) {
				if(num == 13){
					this.form.num = data.train_num;
					this.form.start = data.starting_station;
					this.form.end = data.destination_station;
					this.form.sales = data.sales_station;
					this.form.date = data.date;
					this.form.rates = data.ticket_rates;
					this.form.seat = data.seat_category;
					this.form.seat_num = data.seat_num;
					this.form.name = data.name;
					this.form.id_num = data.id_num;
					this.form.ticket_num = data.ticket_num;
					// 展示识别结果
					this.show = true;
					// 隐藏扫描动画
					this.isShow = false;
		...
		confirm(){
			uni.redirectTo({
			    url: '/pages/ImageTechnology/TrainTicket/TrainTicket'
			});
		}
	}
};
