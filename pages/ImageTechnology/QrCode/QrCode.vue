<template>
	<view>
		...
		<view style="width: 520rpx;height: 60rpx; padding-top: 32rpx; text-align: center; margin: 0 auto;">
			<u-button type="primary" shape="circle" @click="chooseImg">上传二维码图片</u-button>
		</view>
		<view v-if="show">
			...
			<view style="padding-top: 30rpx;background-color: #f2f2f2;">
				<u-cell-group style="width: 100%;"><u-cell-item title="应用场景" title-width="60rpx" value="物品信息管理" :arrow="false"></u-cell-item></u-cell-group>
			</view>
		</view>
	</view>
</template>
...
export default {
	...
	methods: {
		...
		getInfo: function(tk) {
			// 发起请求获取信息
			uni.request({
				url: 'https://aip.baidubce.com/rest/2.0/ocr/v1/qrcode?access_token=' + tk,
				// 请求方式
				method: 'POST',
				// 请求头
				header: {
				...
				success: res => {
					...
					// 单个二维码时
					// 将对象的值循环赋值给数组
					for(var i=0;i<res.data.codes_result[0].text.length;i++){
						this.form.push({value:res.data.codes_result[0].text[i]});
						console.log(i);
					}
					// 展示识别结果
					this.show = true;
		...
		confirm(){
			uni.redirectTo({
			    url: '/pages/ImageTechnology/QrCode/QrCode'
			});
		}
	}
};
