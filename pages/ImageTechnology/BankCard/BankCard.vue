<template>
	<view>
		...
		<view style="width: 520rpx;height: 60rpx; padding-top: 32rpx; text-align: center; margin: 0 auto;">
			<u-button type="primary" shape="circle" @click="chooseImg">上传银行卡正面</u-button>
		</view>
		<view v-if="show">
			...
			<view style="padding-left: 20rpx;padding-right: 30rpx;">
				<u-form :label-style="form">
					<u-form-item :rightIconStyle="{ color: '#888', fontSize: '32rpx' }" :border-bottom="true" label="银 行">
						<u-input :disabled="true" :border="false" placeholder="" v-model="form.name"></u-input>
					</u-form-item>
					<u-form-item :rightIconStyle="{ color: '#888', fontSize: '32rpx' }" :border-bottom="true" label="类 型">
						<u-input :disabled="true" :border="false" placeholder="" v-model="form.type"></u-input>
					</u-form-item>
					<u-form-item :rightIconStyle="{ color: '#888', fontSize: '32rpx' }" :border-bottom="true" label="卡 号">
			...
			<view style="padding-top: 30rpx;background-color: #f2f2f2;">
				<u-cell-group style="width: 100%;"><u-cell-item title="应用场景" title-width="60rpx" value="身份认证、自动绑卡" :arrow="false"></u-cell-item></u-cell-group>
			</view>
		</view>
	</view>
</template>
...
export default {
	...
	methods: {
		...
		getInfo: function(tk) {
			// 发起请求获取信息
			uni.request({
				url: 'https://aip.baidubce.com/rest/2.0/ocr/v1/bankcard?access_token=' + tk,
				// 请求方式
				method: 'POST',
				// 请求头
				header: {
				...
				success: res => {
					...
					// 对识别结果进行判断
					this.getJudge(res.data.result.bank_card_type,res.data.result);
				}
			});
		},
		getJudge: function(type,data) {
			switch (type) {
				// 结果错误时
				case 0:
					// 展示报错信息
				...
				case 1:
					// 给正确结果赋值
					this.form.type = '借记卡';
					this.form.name = data.bank_name;
					this.form.number = res.datacard_number;
					// 展示识别结果
					this.show = true;
					this.isShow = false;
					break;
				...
				// 给正确结果赋值
					this.form.type = '贷记卡';
					this.form.name = data.bank_name;
					this.form.number = data.bank_card_number;
					// 展示识别结果
					this.show = true;
					this.isShow = false;
					break;
		...
		confirm(){
			uni.redirectTo({
			    url: '/pages/ImageTechnology/BankCard/BankCard'
			});
		}
	}
};
