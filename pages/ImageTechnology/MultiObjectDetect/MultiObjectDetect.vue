		...
		<view v-if="show">
			...
			<view style="padding-left: 20rpx;padding-right: 30rpx;">
				<u-form :label-style="form">
					<u-form-item :rightIconStyle="{ color: '#888', fontSize: '32rpx' }" :border-bottom="true" label="距 上">
						<u-input :disabled="true" :border="false" placeholder="" v-model="form.top"></u-input>
					</u-form-item>
					<u-form-item :rightIconStyle="{ color: '#888', fontSize: '32rpx' }" :border-bottom="true" label="距 左">
						<u-input :disabled="true" :border="false" placeholder="" v-model="form.left"></u-input>
					</u-form-item>
					<u-form-item :rightIconStyle="{ color: '#888', fontSize: '32rpx' }" :border-bottom="true" label="宽 度">
						<u-input :disabled="true" :border="false" placeholder="" v-model="form.width"></u-input>
					</u-form-item>
					<u-form-item :rightIconStyle="{ color: '#888', fontSize: '32rpx' }" :border-bottom="true" label="高 度">
						<u-input :disabled="true" :border="false" placeholder="" v-model="form.height"></u-input>
					</u-form-item>
				</u-form>
			</view>
			<view style="padding-top: 30rpx;background-color: #f2f2f2;">
				<u-cell-group style="width: 100%;"><u-cell-item title="应用场景" title-width="60rpx" value="相关推荐、相册分类" :arrow="false"></u-cell-item></u-cell-group>
			</view>
		</view>
	</view>
</template>
...
export default {
	...
	methods: {
		...
		getInfo: function(tk) {
			// 发起请求获取信息
			uni.request({
				url: 'https://aip.baidubce.com/rest/2.0/image-classify/v1/object_detect?access_token=' + tk,
				// 请求方式
				method: 'POST',
				// 请求头
				header: {
				...
				success: res => {
					...
					// 将识别结果赋值给页面值
					this.form.top = res.data.result.top;
					this.form.left = res.data.result.left;
		...
		confirm() {
			uni.redirectTo({
				url: '/pages/ImageTechnology/LicensePlate/LicensePlate'
			});
		}
	}
};
