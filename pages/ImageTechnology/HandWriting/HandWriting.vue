<template>
	<view>
		...
		<view style="width: 520rpx;height: 60rpx; padding-top: 32rpx; text-align: center; margin: 0 auto;">
			<u-button type="primary" shape="circle" @click="chooseImg">上传手写图片</u-button>
		</view>
		<view v-if="show">
			...
			<view style="padding-top: 30rpx;background-color: #f2f2f2;">
				<u-cell-group style="width: 100%;"><u-cell-item title="应用场景" title-width="60rpx" value="手写文档识别" :arrow="false"></u-cell-item></u-cell-group>
			</view>
		</view>
	</view>
</template>
...
export default {
	...
	methods: {
		...
		getInfo: function(tk) {
			// 发起请求获取信息
			uni.request({
				url: 'https://aip.baidubce.com/rest/2.0/ocr/v1/handwriting?access_token=' + tk,
				// 请求方式
				method: 'POST',
				// 请求头
				header: {
				...
				success: res => {
					...
					// 将对象的值循环赋值给数组
					// 参考链接：https://blog.csdn.net/c_o_d_e_/article/details/101349937
		...
		confirm(){
			uni.redirectTo({
			    url: '/pages/ImageTechnology/HandWriting/HandWriting'
			});
		}
	}
};
