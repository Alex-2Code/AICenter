<template>
	<view>
		...
		<view style="width: 520rpx;height: 60rpx; padding-top: 32rpx; text-align: center; margin: 0 auto;">
			<u-button type="primary" shape="circle" @click="chooseImg">上传车牌图片</u-button>
		</view>
		<view v-if="show">
			...
			<view style="padding-left: 20rpx;padding-right: 30rpx;">
				<u-form :label-style="form">
					<u-form-item :rightIconStyle="{ color: '#888', fontSize: '32rpx' }" :border-bottom="true" label="车 牌">
						<u-input :disabled="true" :border="false" placeholder="" v-model="form.number"></u-input>
					</u-form-item>
					<u-form-item :rightIconStyle="{ color: '#888', fontSize: '32rpx' }" :border-bottom="true" label="颜 色">
						<u-input :disabled="true" :border="false" placeholder="" v-model="form.color"></u-input>
					</u-form-item>
				</u-form>
			</view>
			<view style="padding-top: 30rpx;background-color: #f2f2f2;">
				<u-cell-group style="width: 100%;"><u-cell-item title="应用场景" title-width="60rpx" value="无人停车场、违章检测" :arrow="false"></u-cell-item></u-cell-group>
			</view>
		</view>
	</view>
</template>
...
export default {
	...
	methods: {
		...
		getInfo: function(tk) {
			// 发起请求获取信息
			uni.request({
				url: 'https://aip.baidubce.com/rest/2.0/ocr/v1/license_plate?access_token=' + tk,
				// 请求方式
				method: 'POST',
				// 请求头
				header: {
		...
		getJudge: function(erro,rdata) {
			// 识别错误情况
			if(erro == "target recognize error"){
				this.showModal = true;
				this.content = '识别有误！';
			}else{
				// 识别成功时
				let data = rdata.words_result;
				// 将识别结果赋值给页面内容
